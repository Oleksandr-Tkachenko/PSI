...
static FILE* openFile(string* path, const char* options) {
    FILE * f = fopen(path->c_str(), options);
    if (f == NULL) {
        perror(path->c_str());
        exit(EXIT_FAILURE);
    }
...
static bool isEmptyElem(uint8_t * elem, uint size) {
...
static void genRndElem(uint8_t * elem, uint size) {
    int ret = syscall(SYS_getrandom, elem, size, 0);
    if (ret == -1)
        printf("Error generating random element\n");
    else if (ret != size)
        printf("Too less entropy in urandom for generating random element\n");
}

static void printElem(uint8_t * e, uint n){
