...
#define ENC8_K1_round(i) {block1=_mm_aesenc_si128(block1, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block2=_mm_aesenc_si128(block2, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block3=_mm_aesenc_si128(block3, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block4=_mm_aesenc_si128(block4, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block5=_mm_aesenc_si128(block5, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block6=_mm_aesenc_si128(block6, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block7=_mm_aesenc_si128(block7, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block8=_mm_aesenc_si128(block8, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
}

#define ENC8_K1_round_last(i) {block1=_mm_aesenclast_si128(block1, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block2=_mm_aesenclast_si128(block2, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block3=_mm_aesenclast_si128(block3, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block4=_mm_aesenclast_si128(block4, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block5=_mm_aesenclast_si128(block5, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block6=_mm_aesenclast_si128(block6, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block7=_mm_aesenclast_si128(block7, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
	block8=_mm_aesenclast_si128(block8, (*(__m128i const*)(keyptr[0].KEY+i*16))); \
}
...
void intrin_sequential_gen_rnd8_ks1(const uint64_t ctr, uint8_t* CT, uint64_t n_aesiters, ROUND_KEYS* ks){

	ROUND_KEYS *keyptr=(ROUND_KEYS *)ks;
    register __m128i keyA, con, mask, x2, keyA_aux, globAux;
    uint8_t *ctptr;
	uint32_t i, j, ctoffset;

	uint8_t** ctr_buf = (uint8_t**) malloc(sizeof(uint8_t*) * 8);
	//init counter bufs and copy counter
	for(i = 0; i < 8; i++) {
		ctr_buf[i] = (uint8_t*) calloc(16, sizeof(uint8_t));
		*((uint64_t*)ctr_buf[i]) = ctr + i;
	}

	ctoffset = n_aesiters * 16;


	register __m128i inblock1, inblock2, inblock3, inblock4, inblock5, inblock6, inblock7, inblock8,
		block1, block2, block3, block4, block5, block6, block7, block8;

	uint32_t lim = (n_aesiters/8)*8;

	ctptr = CT;
	for(i=0;i<lim; i+=8) {
		inblock1 = _mm_loadu_si128((__m128i const*)(ctr_buf[0]));
		inblock2 = _mm_loadu_si128((__m128i const*)(ctr_buf[1]));
		inblock3 = _mm_loadu_si128((__m128i const*)(ctr_buf[2]));
		inblock4 = _mm_loadu_si128((__m128i const*)(ctr_buf[3]));
		inblock5 = _mm_loadu_si128((__m128i const*)(ctr_buf[4]));
		inblock6 = _mm_loadu_si128((__m128i const*)(ctr_buf[5]));
		inblock7 = _mm_loadu_si128((__m128i const*)(ctr_buf[6]));
		inblock8 = _mm_loadu_si128((__m128i const*)(ctr_buf[7]));

		READ_KEYS1(0)

		block1 = _mm_xor_si128(keyA, inblock1);
		block2 = _mm_xor_si128(keyA, inblock2);
		block3 = _mm_xor_si128(keyA, inblock3);
		block4 = _mm_xor_si128(keyA, inblock4);
		block5 = _mm_xor_si128(keyA, inblock5);
		block6 = _mm_xor_si128(keyA, inblock6);
		block7 = _mm_xor_si128(keyA, inblock7);
		block8 = _mm_xor_si128(keyA, inblock8);
		...
		ENC8_K1_round(8)
		ENC8_K1_round(9)
		ENC8_K1_round_last(10)

		_mm_storeu_si128((__m128i *)(ctptr+0*16), block1);
		_mm_storeu_si128((__m128i *)(ctptr+1*16), block2);
		_mm_storeu_si128((__m128i *)(ctptr+2*16), block3);
		_mm_storeu_si128((__m128i *)(ctptr+3*16), block4);
		_mm_storeu_si128((__m128i *)(ctptr+4*16), block5);
		_mm_storeu_si128((__m128i *)(ctptr+5*16), block6);
		_mm_storeu_si128((__m128i *)(ctptr+6*16), block7);
		_mm_storeu_si128((__m128i *)(ctptr+7*16), block8);

		(*((uint64_t*)ctr_buf[0]))+=8;
		(*((uint64_t*)ctr_buf[1]))+=8;
		(*((uint64_t*)ctr_buf[2]))+=8;
		(*((uint64_t*)ctr_buf[3]))+=8;
		(*((uint64_t*)ctr_buf[4]))+=8;
		(*((uint64_t*)ctr_buf[5]))+=8;
		(*((uint64_t*)ctr_buf[6]))+=8;
		(*((uint64_t*)ctr_buf[7]))+=8;
