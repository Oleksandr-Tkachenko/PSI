...
uint32_t otpsi(role_type role, string* input_path, string* output_path, uint32_t neles, uint32_t pneles,
        uint32_t elebytelen, uint8_t* elements, uint8_t** result, crypto* crypt_env, CSocket* sock, uint32_t ntasks,
        double epsilon, bool detailed_timings) {
    ...
    if (role == SERVER) {
        ...
        otpsi_server(eleptr, input_path, output_path, neles, nbins, pneles,
                internal_bitlen, maskbitlen, crypt_env, sock, ntasks, &prf_state);
#else
        nbins = ceil(epsilon * pneles);
        otpsi_server(eleptr, neles, nbins, pneles,
                internal_bitlen, maskbitlen, crypt_env, sock, ntasks, &prf_state);
#endif
    } else {
#ifdef BILLION_SET
        nbins = neles;
        intersect_size = otpsi_client(eleptr, input_path, output_path,
                neles, nbins, pneles, internal_bitlen, maskbitlen, crypt_env,
                sock, ntasks, &prf_state, &res_pos);
#else
        nbins = ceil(epsilon * neles);
        intersect_size = otpsi_client(eleptr,
                neles, nbins, pneles, internal_bitlen, maskbitlen, crypt_env,
                sock, ntasks, &prf_state, &res_pos);
        cout << "Result has size " << intersect_size << endl;
#endif
...
uint32_t otpsi_client(uint8_t* elements, string* path_hashtable, string* path_output,
        uint32_t neles, uint32_t nbins, uint32_t pneles, uint32_t elebitlen,
        uint32_t maskbitlen, crypto* crypt_env, CSocket* sock, uint32_t ntasks,
        prf_state_ctx* prf_state, uint32_t** result) {

    uint32_t outbitlen = elebitlen, maskbytelen, intersect_size;
    uint8_t *hash_table, *masks;
    uint32_t* nelesinbin;
    uint8_t *server_masks;
    uint32_t* perm = (uint32_t*) calloc(neles, sizeof (uint32_t));
    ...
    //Perform the OPRG execution
    //cout << "otpsi client running ots" << endl;
    oprg_client(hash_table, path_hashtable, path_output, nbins, neles, nelesinbin,
            outbitlen, maskbitlen, crypt_env, sock, ntasks, masks);
...
void otpsi_server(uint8_t* elements, string* path_hashtable, string* path_output,
        uint32_t neles, uint32_t nbins, uint32_t pneles, uint32_t elebitlen, uint32_t maskbitlen,
        crypto* crypt_env, CSocket* sock, uint32_t ntasks, prf_state_ctx* prf_state) {
    uint8_t *hash_table, *masks;
    uint32_t* nelesinbin;
    uint32_t outbitlen, maskbytelen;
...
#endif
    masks = (uint8_t*) malloc(NUM_HASH_FUNCTIONS * neles * maskbytelen);
    oprg_server(hash_table, path_hashtable, path_output, nbins, neles * NUM_HASH_FUNCTIONS, nelesinbin, outbitlen, maskbitlen, crypt_env, sock, ntasks, masks);
    if (DETAILED_TIMINGS) {
        gettimeofday(&t_start, NULL);
    }
#ifdef TIMING
...
void oprg_client(uint8_t* hash_table, uint32_t nbins, uint32_t neles, uint32_t* nelesinbin, uint32_t elebitlen,
    ...
    //TO/DO @Alex: the instantiation of the OT class for the client
    ...
    //TO/DO @Alex: the invocation of the receiver routine is here.
    ...
    //TO/DO @Alex: the client does not need to permute its elements
...
void oprg_client(uint8_t* hash_table, string* path_hashtable, string* path_output,
        uint32_t nbins, uint32_t neles, uint32_t* nelesinbin, uint32_t elebitlen,
        uint32_t maskbitlen, crypto* crypt, CSocket* sock, uint32_t nthreads, uint8_t* res_buf) {
    CBitVector ht_vec, res_vec;
    uint32_t maskbytelen = ceil_divide(maskbitlen, 8);
    ...
    cout << "Client: bins = " << nbins << ", elebitlen = " << elebitlen << " and maskbitlen = " <<
            maskbitlen << " and performs " << nbins << " OTs" << endl;
    cout << "Input file path = " << path_hashtable->c_str() << endl;
    cout << "Output file path = " << path_output->c_str() << endl;
#endif
    ...
    //sock = socket object for communication
    receiver = new KKOTExtRcv(m_nCodeWordBits, elebitlen, crypt, sock);
    receiver->elem_size = 10;
    receiver->openFiles(path_hashtable, path_output);
...
void oprg_server(uint8_t* hash_table, uint32_t nbins, uint32_t totaleles, uint32_t* nelesinbin, uint32_t elebitlen,
    ...
    //TO/DO @Alex: the instantiation of the OT class for the server
    //m_nCodeWordBits = 512 (currently constant)
    //elebitlen = bit-length of elements in the hash table. needs to be set < 77
    //crypt = crypto class object, which is inistialized with 128
    //sock = socket object for communication
    ...
    //TO/DO @Alex: the invocation of the sender routine is here.
    //nbins = number of bins in the hash table
    //maskbitlen = output bit-length of the OT (=log_2(3*|X|) + log_2(|Y|) + 40), can be larger, though
    //ht_vec = the hash table. Needs to be replaced by a file access
    //res_vec = the output buffer for the masks. needs to be replaced by a file access
    ...
    //TO/DO @Alex: after the sender is done, the elements in the output file need to be permuted. 
...
void oprg_server(uint8_t* hash_table, string* path_hashtable, string* path_output,
        uint32_t nbins, uint32_t totaleles, uint32_t* nelesinbin, uint32_t elebitlen,
        uint32_t maskbitlen, crypto* crypt, CSocket* sock, uint32_t nthreads, uint8_t* res_buf) {
    CBitVector ht_vec, res_vec;
    KKOTExtSnd* sender;
    uint32_t maskbytelen = ceil_divide(maskbitlen, 8);
    ...
    //sock = socket object for communication
    sender = new KKOTExtSnd(m_nCodeWordBits, elebitlen, crypt, sock);
    sender->openFiles(path_hashtable, path_output);
    sender->elem_size = 11;
#ifndef BILLION_SET
    ht_vec.AttachBuf(hash_table, totaleles * ceil_divide(elebitlen, 8));
    res_vec.AttachBuf(res_buf, totaleles * maskbytelen);
